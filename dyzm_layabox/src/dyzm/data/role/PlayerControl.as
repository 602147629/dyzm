	...
	public class PlayerControl extends RoleVo
	{
		...
		public function setSkill(id:int):Boolean
		{
			...
			if (curState == RoleState.STATE_NORMAL && isRuning == false){ // 地面正常状态
				curForm = 1;
				bindObj = keyToSkill.floorBind;
				skillObj = keyToSkill.floorVo;
			}else if (curState == RoleState.STATE_NORMAL && isRuning == true){ // 地面跑步状态
				curForm = 2
				bindObj = keyToSkill.runBind;
				skillObj = keyToSkill.runVo;
				...
				curForm = 3
				bindObj = keyToSkill.skyBind;
				skillObj = keyToSkill.skyVo;
			}else{
				return false;
			}
		...
		{
			if (curSkill as SkillBlock && !HandleView.skillKeyDownInfo[keyToSkill.blockId]){
				curSkill.end();
			}
			super.frameUpdate();
		}
